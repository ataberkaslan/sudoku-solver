...
function App() {
  ...
  function solveStep(num){
    ...
    if (solveSudoku(newGrid) && isValidGrid(newGrid)) {
      ...
      const filtered = steps.filter(val => val.value == num);
      if(filtered.length){
        setActiveGrid([filtered[0].row,filtered[0].col])
        tempGrid[filtered[0].row][filtered[0].col] = filtered[0].value;
